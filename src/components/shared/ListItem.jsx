...
const handleCheckbox=() =>setChecked(!isChecked)
    return (
        <View className={`border-[2px] border-white ${isChecked && 'border-main border-[2px]'}  bg-white rounded-[6px] mt-[10px] px-[10px] py-[10px] `}>
           ...
           detail && (
            <View className='mt-[10px] border-t-[1px] border-border pt-[10px] flex-row justify-between'>
              <Pressable className='flex-row items-center gap-[5px]' onPress={() => { console.log('selam') }}>
                <Ionicons name="information-circle" size={16} color="#343440" />
                <Text className='text-primary text-[13px]'>Detay</Text>
              </Pressable>
              <Pressable className='flex-row items-center gap-[5px]' onPress={() => { deleteTask(item.id)}}>
